...
//$t->is($request->getPathInfo(), '/test/klaus', '->getPathInfo() returns the url path value if it not exists use default REQUEST_URI');

$request = new myRequest($dispatcher);
$_SERVER['QUERY_STRING'] = 'test';
$_SERVER['REQUEST_URI']  = '/frontend_test.php/test/klaus2?test';
//$t->is($request->getPathInfo(), '/test/klaus', '->getPathInfo() returns the url path value if it not exists use default REQUEST_URI without query');

$request->resetPathInfoArray();
$request = new myRequest($dispatcher);
unset($_SERVER['PATH_INFO'], $_SERVER['REQUEST_URI'], $_SERVER['QUERY_STRING'], $_SERVER['SCRIPT_NAME']);
